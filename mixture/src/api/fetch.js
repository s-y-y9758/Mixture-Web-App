...
export function getVideoer(id,page,count) {//获取图片里每个摄影师对应的数据，来自图虫api
	if(!page){
		page=1
	}
	if(!count){
	...
	let mine = new Promise((resolve,reject)=>{
		axios.get(`/rest/sites/${id}/posts?page=${page}&count=${count}`)
    	.then(res=>{
          	resolve(res.data)
		})
	})
...
export function getTags(name) {//获取图片里每个摄影师对应的数据，来自图虫api
	let mine = new Promise((resolve,reject)=>{
		axios.get(`/rest/tags/${name}/posts?page=1&count=20&order=weekly`)
    	.then(res=>{
          	resolve(res.data)
		})
	})
