	...
	<scroll :data='getVideo'>
		<div>
			<div class='part' v-for='item,index in getVideo'>
				...
				<div class='part_main'>
					<img 
					:src='item.data.cover&&item.data.cover.detail' @touchstart='play($event,index)' @touchend='touchesend($event,index)' 
					ref='images'>
					<video 
					ref='video' 
					class='video' 
					controls 
					@touchstart='play($event,index)' @touchend='touchesend($event,index)'>
